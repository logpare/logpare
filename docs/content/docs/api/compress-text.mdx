...
`compressText()` takes a complete string, so the file is read in full first. For genuine
streaming — feeding lines in as they arrive — use
[`createDrain()`](/docs/api/create-drain) instead.
