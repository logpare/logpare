...
description: Complete TypeScript type definitions for logpare
---
...
- `format` - Output format: `'summary'`, `'detailed'`, `'json'`, or `'json-stable'`
  (default: `'summary'`). `'json-stable'` sorts keys and omits volatile fields for
  stable diffs and LLM KV-cache hits.
...
- `'json-stable'` - Cache-optimized JSON with sorted keys and volatile fields omitted,
  for maximum LLM KV-cache hits and stable diffs
