...
description: What logpare does, the problem it solves, and where to go next — start here.
---
...
- **High compression rates**: 60-90% token reduction on repetitive logs
...
- **Multiple output formats**: Summary, detailed, JSON, and deterministic JSON
...
repeated lines into a pattern with counts and sampled values, so the original text cannot
be reconstructed from the output. Keep your raw logs; send the compressed view to the
model.
